...
//   2. Read current HEAD as the parent (may fail for first commit — that's OK)
//   3. Fill in the Commit struct: tree, parent, author, timestamp, message
//   4. Serialize the commit struct to text with commit_serialize()
//   5. Write the serialized text as an OBJ_COMMIT with object_write()
//   6. Update HEAD to point to the new commit with head_update()
//   7. Store the new commit's ID in *commit_id_out
//
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // Step 2: Try to read the current HEAD as parent (optional — first commit has none)
    Commit c;
    memset(&c, 0, sizeof(c));
    ...
    // Step 3: Fill in the commit struct
    c.tree      = tree_id;
    ...
    // Step 4: Serialize the commit struct to its text representation
    ...
    // Step 5: Write to the object store as OBJ_COMMIT
    ObjectID commit_id;
    ...
    // Step 6: Atomically update HEAD (branch ref) to the new commit
    if (head_update(&commit_id) != 0) {
        fprintf(stderr, "error: failed to update HEAD\n");
        return -1;
    }

    // Step 7: Return the new commit ID to the caller
