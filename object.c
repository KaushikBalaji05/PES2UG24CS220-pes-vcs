...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // Write the full object (header + null + data)
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 6: Allocate buffer and copy the data portion (after the '\0')
    uint8_t *data_start = null_byte + 1;
    size_t actual_data_len = (size_t)file_size - header_len - 1;

    void *out = malloc(actual_data_len + 1); // +1 for safety null terminator
    if (!out) {
        free(raw);
        return -1;
    }
    memcpy(out, data_start, actual_data_len);
    ((uint8_t *)out)[actual_data_len] = '\0'; // safe for text objects
